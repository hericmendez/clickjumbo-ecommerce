...
<div class="table-responsive">  <table class="table table-striped table-hover align-middle">
    ...
    <script>
        ...
        function gerarAcoesDropdown(pedido) {
            return `
    <div class="dropdown">
  <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false z-1">
    Ações
  </button>
  <ul class="dropdown-menu dropdown-menu-end">
    <li><a class="dropdown-item" href="#" onclick="verDetalhes(${pedido.id})">Ver detalhes</a></li>
    <li><a class="dropdown-item" href="#" onclick="alterarStatus(${pedido.id})">Alterar status</a></li>
    <li><a class="dropdown-item" href="#" onclick="baixarPDFPorId(${pedido.id})">Gerar PDF</a></li>
    <li><a class="dropdown-item" href="#" onclick="baixarComprovante(${pedido.id})">Comprovante</a></li>
    <li><a class="dropdown-item text-danger" href="#" onclick="deletarPedido(${pedido.id})">Excluir</a></li>
  </ul>
</div>
`;
        }
        ...
        function renderizarPedidos(lista) {
            ...
            lista.forEach(pedido => {
                ...
                tr.innerHTML = `
                        ...
                        <td><span class="badge text-bg-${pedido.status}">${pedido.status}</span></td>
  ...
  cancelled: 'danger'
                ...
                lista.forEach(pedido => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td>${pedido.penitenciaria?.nome || '—'}</td>
                        <td>R$ ${pedido.total}</td>
                        <td><span class="badge text-bg-${statusClass[pedido.status] || 'secondary'}">${pedido.status}</span></td>
                        <td>${formatarData(pedido.data)}</td>
                        <td>${gerarAcoesDropdown(pedido)}</td>
                    `;
                    tbody.appendChild(tr);
        ...
        function alterarStatus(id) {
            ...
            fetch('https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/cancel-order', {
                method: 'POST',
                headers: {
        ...
        function deletarPedido(id) {
            ...
            fetch(`https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/orders/${id}`, {
                method: 'DELETE',
                credentials: 'include',
                headers: {
