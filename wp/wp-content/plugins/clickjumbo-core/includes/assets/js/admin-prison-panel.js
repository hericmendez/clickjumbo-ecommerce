...
async function carregarTabela() {
  ...
  try {
    const res = await fetch("https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/prison-list-full");
...
async function verProdutos(slug) {
  ...
  try {
    const resPrison = await fetch(
      `https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/prison-details/${slug}`
    );
    const prisonData = await resPrison.json();
    const nomePenitenciaria = prisonData.content?.nome || slug;

    const res = await fetch(
      `https://clickjumbo.com.brhttps://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/product-list?slug=${slug}`,
      {
        headers: {
...
<td>
  ${prod.thumb ? `<img src="${prod.thumb}" alt="${prod.name}" style="max-width: 60px; height: auto;" />` : '—'}
  <br>
  ${JSON.stringify(prod.name)}
                ...
                <td>${
                  `${prod.weight.toFixed(2).replace(".", ",")}kg` || "—"
                       ...
                       <td>${`${prod.sku || "-"}`}</td>
                            <td>${`${prod.maxUnitsPerClient || 1}`}</td>
...
async function verDetalhesProduto(id) {
  ...
  try {
    const res = await fetch(`https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/product-details/${id}`, {
      credentials: "include",
    ...
    <p><strong>Peso:</strong> ${
      parseFloat(content.weight).toFixed(2) || "—"
    } kg</p>
...
function deleteProduct(id) {
  ...
  fetch(`https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/delete-product/${id}`, {
    method: "DELETE",
    credentials: "include",
    headers: { "X-WP-Nonce": clickjumbo_data.nonce },
  })
...
async function editPrison(slug) {
  const res = await fetch(`https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/prison-details/${slug}`, {
    credentials: "include",
    headers: { "X-WP-Nonce": clickjumbo_data.nonce },
  });
...
async function deletePrison(slug) {
  ...
  await fetch(`https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/delete-prison/${slug}`, {
    method: "DELETE",
    credentials: "include",
    headers: { "X-WP-Nonce": clickjumbo_data.nonce },
  });
...
form.onsubmit = async (e) => {
  ...
  const url = modoEdicao
    ? `https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/update-prison/${modoEdicao}`
    : `https://clickjumbo.com.br/wp/wp-json/clickjumbo/v1/register-prison`;
