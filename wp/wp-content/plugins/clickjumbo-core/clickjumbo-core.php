...
add_action('admin_enqueue_scripts', 'clickjumbo_enqueue_bootstrap_admin');
function clickjumbo_enqueue_bootstrap_admin($hook) {
    // Carregar somente nas páginas do plugin (evita afetar o painel inteiro)
    if (strpos($hook, 'clickjumbo') === false) {
        return;
    }

    // Bootstrap 5 (CDN)
    wp_enqueue_style('bootstrap-5', 'https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css');
    wp_enqueue_script('bootstrap-5', 'https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js', [], null, true);
}
